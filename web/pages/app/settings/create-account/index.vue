...
import { isValidEmailForDomain } from "@utils/regex";

definePageMeta({
  title: "Configurações / Criar conta",
...
const validationSchema = z.object({
  ...
  email: z
    ...
    .transform((value) => isValidEmailForDomain(value, COMPANY.value.website)),
...
const onSubmit = async () => {
  ...
  if (isValid.value) {
    const { data, status, error } = await useFetch(`${baseURL}/users`, {
      method: "POST",
      headers: createHeader(),
      body: {
